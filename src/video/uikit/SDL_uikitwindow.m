...
//ME
    ...
    if (window->flags & SDL_WINDOW_ME)
       data.viewcontroller = [[SDL_uikitviewcontroller_2 alloc] initWithSDLWindow:window];
    else
...
//          return SDL_SetError("Only one window allowed per display.");
        ...
        if (data.uiscreen == [UIScreen mainScreen]) {
            if (window->flags & (SDL_WINDOW_FULLSCREEN|SDL_WINDOW_BORDERLESS)) {
                ...
                [UIApplication sharedApplication].statusBarHidden = YES;//ME NO;
            }
        }
#endif /* !TARGET_OS_TV */
...
UIKit_ShowWindow(_THIS, SDL_Window * window)
{
    @autoreleasepool {
        SDL_WindowData * data = (__bridge SDL_WindowData *) window->driverdata;
/*       UIWindow * uiwindow;
        uiwindow = data.uiwindow;

        if (window->flags & SDL_WINDOW_ME)
            if([uiwindow rootViewController]==nil) {
                PlaylistController * plc = [[PlaylistController alloc] init];
                UITableView * tv = [[UITableView alloc] init];
                todo
                plc->tv;
                uiwindow.rootViewController = plc;
            }
//ME
    ...
    @autoreleasepool {
        if (window->driverdata != NULL) {
            ...
            for (SDL_uikitview *view in views) {
                ...
                if ([className containsString:@"SDL_"])
