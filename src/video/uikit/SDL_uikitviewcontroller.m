...
- (void)loadView
{
    ...
    [win addGestureRecognizer:tapGestureRecognizer];
...
- (void)lblClick:(UITapGestureRecognizer *)tapGesture {
    char * title = self.window->title;
    if(title==NULL) return;
    
    char * key = "http";
    char * pch;
    pch = strpbrk (title, key);
    if(pch!=NULL) {
        self.window->flags |= SDL_WINDOW_QUIT;
//      SDL_VideoQuit();
//      SDL_SetVideoUrl(NULL);
        SDL_Event event;
        event.type = SDL_QUIT;
        int posted = (SDL_PushEvent(&event));
        posted = posted + 1;//suppress warning
    }
}
...
{
//ME    BOOL hidden = (window->flags & (SDL_WINDOW_FULLSCREEN|SDL_WINDOW_BORDERLESS)) != 0;
//ME    return hidden;
    return true;
}
