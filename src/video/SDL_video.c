...
//ME
int
SDL_SetVideoUrl(char *video_url)
{
    /* Check to make sure we don't overwrite '_this' */
    if (_this == NULL) {
        return -1;
...
//ME
SDL_Window *
SDL_CreateWindow2(const char *title, int x, int y, int w, int h, Uint32 flags)
{
    SDL_Window *window;
    ...
    window->flags = ((flags & CREATE_FLAGS) | SDL_WINDOW_HIDDEN | SDL_WINDOW_ME);
    window->last_fullscreen_flags = window->flags;
    window->opacity = 1.0f;
...
//
void
SDL_EraseWindow(SDL_Window * window)
{
//TODO
}
...
//ME
int
SDL_GetWindowQuit(SDL_Window * window, int * out_opacity)
{
    if(window->flags & SDL_WINDOW_QUIT) {
        *out_opacity = 1;
        return 0;
    }
...
//ME
int
SDL_GetWindowPlay(SDL_Window * window, int * out_opacity)
{
    if(window->flags & SDL_WINDOW_PLAY) {
        *out_opacity = 1;
        return 0;
    }
